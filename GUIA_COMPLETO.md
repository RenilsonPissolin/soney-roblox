# 📚 GUIA COMPLETO E DEFINITIVO — SONEY + NEXUS + 12 ROBÔS
## Todos os scripts, passo a passo, sem faltar nada

---

# 🇦 REPLICATEDSTORAGE — MÓDULOS COMPARTILHADOS

## 📦 Script 1: ConfigModule (ModuleScript)

**Local:** `ReplicatedStorage > ConfigModule`
...
## 📦 Script 2: SoneyAPIBridge (ModuleScript)

**Local:** `ReplicatedStorage > SoneyAPIBridge`
...
function SoneyAPI.syncPlayer(player, amount, reason)
    ...
    if s then return true, HttpService:JSONDecode(r) end; return false, nil
end
...
## 📦 Script 3: EpisodeService (ModuleScript)

**Local:** `ReplicatedStorage > EpisodeService`
...
function EpisodeService.getCurrentEpisode()
    ...
    return false, { id = "ep-1", number = 1, title = "O Primeiro Episódio", hook = "O segredo...", choices = {{id="A",text="Revelar"},{id="B",text="Guardar"}} }
end
...
function EpisodeService.submitVote(player, choice)
    ...
    if s then lastFetch = 0; return true, r end; return false, nil
end
function EpisodeService.clearCache() currentEpisode = nil; lastFetch = 0 end
return EpisodeService
```

## 📦 Script 4: BCEvent (RemoteEvent)

**Local:** `ReplicatedStorage > BCEvent`

Criar como **RemoteEvent** (não script):
1. Botão direito em ReplicatedStorage → **Insert Object** → **RemoteEvent**
2. Renomeia para **BCEvent**

## 📦 Script 5: ToolEvent (RemoteEvent)

**Local:** `ReplicatedStorage > ToolEvent`

1. Botão direito → **Insert Object** → **RemoteEvent**
2. Renomeia para **ToolEvent**

## 📦 Script 6: SoneyNotifyEvent (RemoteEvent)

**Local:** `ReplicatedStorage > SoneyNotifyEvent`

1. Botão direito → **Insert Object** → **RemoteEvent**
2. Renomeia para **SoneyNotifyEvent**

---

# 🇧 SERVERSCRIPTService — 9 SCRIPTS DE SERVIDOR

## 📜 Script 7: LeaderstatsSystem (Script)

**Local:** `ServerScriptService > LeaderstatsSystem`
...
## 📜 Script 8: NexusEconomy (Script)

**Local:** `ServerScriptService > NexusEconomy`
...
function addBC(player, amount, reason)
    ...
    if notifyEvent then notifyEvent:FireClient(player, {title="💰 +"..amount.." BC", message=reason, duration=3}) end
    return true
end
...
## 📜 Script 9: FerramentasMineracao (Script)

**Local:** `ServerScriptService > FerramentasMineracao`
...
## 📜 Script 10: NexusMonetizacao (Script)

**Local:** `ServerScriptService > NexusMonetizacao`
...
## 📜 Script 11: PurchaseHandler (Script)

**Local:** `ServerScriptService > PurchaseHandler`
...
local function processReceipt(r)
    ...
    if coins and _G.SoneyAwardCoins then
        _G.SoneyAwardCoins(p, coins, "purchase"); processed[r.PurchaseId] = true
        return Enum.ProductPurchaseDecision.PurchaseGranted
    end
    return Enum.ProductPurchaseDecision.NotProcessedYet
end
...
## 📜 Script 12: VoteHandler (Script)

**Local:** `ServerScriptService > VoteHandler`
...
## 📜 Script 13: UINotifier (Script)

**Local:** `ServerScriptService > UINotifier`

```lua
local Players = game:GetService("Players"); local ReplicatedStorage = game:GetService("ReplicatedStorage")
local notifyEvent = Instance.new("RemoteEvent"); notifyEvent.Name = "SoneyNotifyEvent"; notifyEvent.Parent = ReplicatedStorage
...
## 📜 Script 14: SoneyCutsceneHandler (Script)

**Local:** `ServerScriptService > SoneyCutsceneHandler`
...
## 🖥️ Script 15: NexusHUD (LocalScript)

**Local:** `StarterGui > NexusHUD`

```lua
local Players = game:GetService("Players"); local ReplicatedStorage = game:GetService("ReplicatedStorage")
local player = Players.LocalPlayer
local screenGui = Instance.new("ScreenGui"); screenGui.Name = "NexusHUD"; screenGui.ResetOnSpawn = false; screenGui.Parent = player:WaitForChild("PlayerGui")

-- Painel Bio-Créditos
local panel = Instance.new("Frame"); panel.Size = UDim2.new(0,200,0,50); panel.Position = UDim2.new(1,-220,0,10)
panel.BackgroundColor3 = Color3.fromRGB(5,5,20); panel.BackgroundTransparency = 0.2; panel.BorderSizePixel = 0; panel.Parent = screenGui
local corner = Instance.new("UICorner"); corner.CornerRadius = UDim.new(0,10); corner.Parent = panel
local stroke = Instance.new("UIStroke"); stroke.Color = Color3.fromRGB(0,200,255); stroke.Thickness = 1.5; stroke.Transparency = 0.5; stroke.Parent = panel
...
## 🖥️ Script 16: SoneyClientUI (LocalScript)

**Local:** `StarterGui > SoneyClientUI`
...
## 🖥️ Script 17: VoteUI (LocalScript)

**Local:** `StarterGui > VoteUI`
...
btn.MouseButton1Click:Connect(function()
    if main.Visible then main.Visible = false; overlay.Visible = false
    else overlay.Visible = true; main.Visible = true; loadEpisode() end
end)
...
## 🖥️ Script 18: SoneyShowroom (LocalScript)

**Local:** `StarterGui > SoneyShowroom`
...
# 🇩 WORKSPACE — MODELOS E SCRIPTS

## 🏗️ BossTitan (Script) — Colocar dentro do Model TitanEsmeralda

**Workspace > TitanEsmeralda > BossTitan (Script)**
...
local MAX_HEALTH = 5000; local currentHealth = MAX_HEALTH; local currentPhase = 1; local active = true; local playersInFight = {}
...
function advancePhase()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        notifyEvent:FireClient(p, {title="🌿 FASE "..currentPhase.." — "..phase.item, message=phase.reward.." BC liberados!", duration=5})
    end
...
task.spawn(function() while true do task.wait(3600); if not boss.Parent then
    local nb = boss:Clone(); nb.Parent = workspace; nb:FindFirstChild("Core").Position = Vector3.new(0,50,0); print("🌿 Boss renasceu!") end end end)
print("🌿 BossTitan ativo — 5 fases!")
```

## 🏗️ EnemyPatrol (Script) — Colocar dentro de CADA Inimigo1 a Inimigo12

**Workspace > Inimigo1 > EnemyPatrol (Script)**
...
## 🏗️ ReliquiaInterativa (Script) — Colocar dentro de CADA relíquia

**Workspace > Reliquias > Reliquia1 > Script**
...
## 🏗️ EasterEggSP (Script) — Colocar dentro da Estátua de São Paulo

**Workspace > EstatuaSP > Script**
...
## 🏗️ ZonaVenda (Script) — Colocar dentro da ZonaDeVenda

**Workspace > ZonaDeVenda > Script**
...
# 🇪 ESTRUTURA FINAL COMPLETA NO EXPLORER
...
📦 ReplicatedStorage
 ...
 ┣ 📜 BCEvent (RemoteEvent)
 ┣ 📜 ToolEvent (RemoteEvent)
 ┗ 📜 SoneyNotifyEvent (RemoteEvent)

📦 ServerScriptService
 ┣ 📜 LeaderstatsSystem (Script)
 ┣ 📜 NexusEconomy (Script)
 ┣ 📜 FerramentasMineracao (Script)
 ┣ 📜 NexusMonetizacao (Script)
...
📦 StarterGui
 ┣ 📜 NexusHUD (LocalScript)
 ┣ 📜 SoneyClientUI (LocalScript)
 ┣ 📜 VoteUI (LocalScript)
 ┗ 📜 SoneyShowroom (LocalScript)

📦 Workspace
 ┣ 📦 SpawnLocation (ponto de spawn)
 ┣ 📦 ZonaDeVenda (Part + Script)
 ┣ 📦 Reliquias (Folder)
 ┃  ┗ 📦 Reliquia1..20 (Part + ProximityPrompt + Script)
 ┣ 📦 RotaInimigo1..12 (Folders com waypoints)
 ┣ 📦 Inimigo1..12 (Models)
 ┃  ┣ Body (Part), Head (Part), PointLight
 ┃  ┗ EnemyPatrol (Script)
 ┣ 📦 TitanEsmeralda (Model)
 ┃  ┣ Core (Part), HealthBar (Part)
 ┃  ┣ Bulbo1..5 (Parts)
 ┃  ┗ BossTitan (Script)
 ┗ 📦 EstatuaSP (Model)
...
**📋 TOTAL: 22 scripts** — 3 ModuleScripts + 8 Scripts servidor + 4 LocalScripts cliente + 7 Scripts workspace

**Pronto!** É só seguir essa ordem e colar cada script no lugar certo. 🚀🔥